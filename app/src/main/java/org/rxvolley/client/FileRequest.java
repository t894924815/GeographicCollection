package org.rxvolley.client;

import android.text.TextUtils;
import android.util.Log;

import org.rxvolley.http.HttpHeaderParser;
...
public class FileRequest extends Request<byte[]> {
    ...
    public byte[] handleResponse(URLHttpResponse response) throws IOException {
        ...
        if (fileSize <= 0) {
            Log.e("error","Response doesn't present Content-Length!");
        }
        ...
        if (isSupportRange) {
            ...
            if (!TextUtils.isEmpty(realRangeValue)) {
                ...
                if (TextUtils.indexOf(realRangeValue, assumeRangeValue) == -1) {
                    Log.e("error","The Content-Range Header is invalid Assume["
        ...
        try {
            ...
            try {
                ...
                Log.e("error","Error occured when calling consumingContent");
            }
            tmpFileRaf.close();
        }
        return null;
